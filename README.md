...
这是一个 Spring Boot 应用，用于处理 ACME HTTP-01 挑战验证，支持 acme.sh 的域名验证。

## 项目结构

```
.
├── src/main/java/com/example/acme/
│   ├── AcmeChallengeApplication.java    # Spring Boot 主类
│   └── controller/
│       ├── AcmeChallengeController.java # ACME 挑战控制器
...
├── start-acme-test.sh                   # 启动脚本
└── README.md                            # 项目说明
```

## 功能特性

- ✅ 支持 ACME HTTP-01 挑战验证
- ✅ 自动创建 `.well-known/acme-challenge` 目录
- ✅ 支持文件系统和内存两种响应方式
- ✅ 提供健康检查和调试端点
- ✅ Docker 容器化部署
- ✅ 与 step-ca 和 acme.sh 集成
...
# 2. 运行应用
java -jar target/acme-challenge-server-1.0.0.jar
...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | `/.well-known/acme-challenge/{token}` | 处理 ACME HTTP-01 挑战 |
| POST | `/.well-known/acme-challenge/{token}` | 添加挑战响应到内存 |
| DELETE | `/.well-known/acme-challenge/{token}` | 删除内存中的挑战响应 |
| GET | `/.well-known/acme-challenge/health` | 健康检查 |
| GET | `/.well-known/acme-challenge/debug/all` | 查看所有挑战响应 |
...
|------|------|------|
| GET | `/test` | 基本测试 |
| GET | `/test/acme/{token}` | 模拟 ACME 挑战 |
| GET | `/test/port-check` | 端口检查 |
...
| 变量名 | 默认值 | 描述 |
|--------|--------|------|
| `acme.challenge.webroot` | `/tmp/webroot` | ACME 挑战文件根目录 |
...
1. acme.sh 向 step-ca 申请证书
2. step-ca 返回 HTTP-01 挑战
3. acme.sh 在 `/tmp/webroot/.well-known/acme-challenge/` 创建验证文件
4. Spring Boot 应用响应挑战请求
5. step-ca 验证域名所有权
6. step-ca 签发证书
...
# 检查容器网络
docker network ls
docker network inspect acme_acme-network

# 检查服务连通性
docker exec acmesh ping acme-challenge-server
```
...
```java
@GetMapping("/{token}")
public ResponseEntity<String> handleChallenge(@PathVariable String token) {
    // 实现新的挑战逻辑
...
public ResponseEntity<String> handleChallenge(@PathVariable String token) {
    // 自定义响应格式
    String response = generateCustomResponse(token);
    return ResponseEntity.ok(response);
}
```
...
public class AcmeChallengeController {
    ...
    @GetMapping("/{token}")
    public ResponseEntity<String> handleChallenge(@PathVariable String token) {
        log.info("Processing ACME challenge for token: {}", token);
        // 处理逻辑
    }
}
```
